class SendRequestDetailController {
    constructor($scope, API, $auth, $sce, $stateParams, $state, $compile, DTOptionsBuilder, DTColumnBuilder, SAAPI, $window, AclService, $location) {
        ...
        this.dtColumns = [
            ...
            DTColumnBuilder.newColumn(null).withTitle('Opened').renderWith(function (data) {
                if (data.open_status == 1) { return `<i class="fa fa-check"></i>` }
                else { return `` }
            }).notSortable(),
            DTColumnBuilder.newColumn(null).withTitle('Clicked').renderWith(function (data) {
                if (data.click_status == 1) { return `<i class="fa fa-check"></i>` }
                else { return `` }
            }).notSortable(),
...
export const SendRequestDetailComponent = {
    templateUrl: './views/app/pages/email-marketing/action-funnel/send-email-sms-component.html',
    controller: SendRequestDetailController,
    controllerAs: 'vm',
    bindings: {}
}