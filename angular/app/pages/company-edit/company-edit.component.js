class CompanyEditController {
    constructor($stateParams, $state, SAAPI, uploads, $scope, $http, $window) {
        'ngInject'
        this.$state = $state
        $scope.country = {};
        ...
        $scope.notShowsitebuilder = false
        ...
        this.save = function (isValid) {
            ...
            delete this.companyeditdata.data.logo;
    ...
    save(isValid) {
        ...
        if (isValid) {
            ...
            let fd = new FormData();
            let $state = this.$state
            this.companyeditdata.one(companyId).put()
                .then(() => {
                    error_file = true;
                    let alert = { type: 'success', 'title': 'Success!', msg: 'Company settings has been updated.' }
                    $state.go($state.current, { alerts: alert })
            ...
            if (this.company_logo != undefined) {
                ...
                let ComapnySettings = SAAPI.service('update-company-logo', SAAPI.all('company'));
...
export const CompanyEditComponent = {
    templateUrl: './views/app/pages/company-edit/company-edit.component.html',
